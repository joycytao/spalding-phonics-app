...
Spelling Tests play one randomly selected approved whole-word recording for each phonogram. When more than one approved word is available, the next prompt avoids repeating the immediately previous word; a single-word pool may repeat. The word remains hidden until the learner selects `Check`, then the existing self-assessment flow reveals it.
