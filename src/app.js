...
function renderCard() {
  ...
  const reviewPracticeNavigation = getReviewPracticeNavigationAction(state.session);
  const navigationAction = state.session.mode === 'review-practice' ? reviewPracticeNavigation : practiceNavigation;
  const isPracticeLike = shouldAutoPlayForMode(state.session.mode);
  const showPlaybackControl = !isPracticeLike || shouldShowPracticePlaybackControl(state.audioState);
...
function startSession(ids, mode) {
  ...
  render();
  scheduleReviewPracticeAutoPlay();
}

function scheduleReviewPracticeAutoPlay() {
  cancelAutoPlay();
  if (!shouldAutoPlayForMode(state.session?.mode)) return;
  cancelAutoPlay = schedulePracticePlayback(() => {
    cancelAutoPlay = () => {};
    if (state.screen === 'session' && shouldAutoPlayForMode(state.session?.mode) && state.audioState === 'idle') listen();
  });
}
...
app.addEventListener('click', (event) => {
  ...
  if (action === 'home') { cancelAutoPlay(); cancelAutoPlay = () => {}; stopAudio(); state = { ...state, screen: 'home' }; render(); }
  ...
  if (action === 'finish') { cancelAutoPlay(); cancelAutoPlay = () => {}; stopAudio(); state = { ...state, screen: 'home', session: null }; render(); }
  if (action === 'next') { cancelAutoPlay(); cancelAutoPlay = () => {}; stopAudio(); const next = advance(state.session); if (next.isComplete) { if (next.mode === 'review-practice') { startSession(next.items.map((item) => item.id), 'review-exam'); return; } state = { ...state, session: next, screen: next.mode === 'practice' ? 'home' : 'result' }; } else { state = { ...state, session: next, heard: false, audioState: 'idle', checked: false, error: '' }; } render(); scheduleReviewPracticeAutoPlay(); }
